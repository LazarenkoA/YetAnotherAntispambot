...
func Test_prompt(t *testing.T) {
	...
	assert.Equal(t, `Ты модератор IT чата. В чате запрещен наем сотрудников. Зашел новый участник и отправил новое сообщение, произведи анализ сообщения из чата и оцени вероятность того, что оно является спамом.
...
func Test_GetCommitMsg(t *testing.T) {
	...
	t.Run("pass", func(t *testing.T) {
		...
		client.EXPECT().ChatWithContext(gomock.Any(), gomock.Any()).Return(&gigachat.ChatResponse{
			Choices: []gigachat.Choice{{Message: gigachat.Message{Content: "true|89|в сообщении фигурирует фраза про криптовалюту и заработок"}}},
		}, nil)
